This repository contains the source code for the tundev-shell, it is part of the remote access project.
...
This shell is running on the RDV server, and implemented in python
For more information about the tundev-shell, please consult the related Confluence page at:
https://www.bwiki.bticino.it:8443/display/PRJACCESDISTANT/Architecture+and+technical+documentation#Architectureandtechnicaldocumentation-tundev-shell
...
- Remove the concept of username in vtun_manager.py (and replace it with something more generic, for example tundev_id)
...
- Remove hardcoded list of profiles and rpi* accounts from vtun_manager.py
  Code has been cleaned-up and can now use an external database since commit ed79c20d6aadaf3f0e8bb762cc64bd1eefa01574
...
- On PythonVtunLib, improve server-side tunnel management
  ...
  * the server side of a tunnel: it is the server part of a tunnel that matches a session in the vtund deamon configuration file.
  ...
  Future version of the project might implements a way to keep only one deamon for all tunnels, since the deamon can reload its configuration file, there should be a way to add/remove tunnels, regenerate configuration file and tell the deamon to reload this file.
